...
(function () {
  ...
  (function () {
    ...
    let _sendId = '';
  let _queuePayload = { html: '', plain: '', attachments_b64: [], inline_images_b64: [] };
    ...
    (function() {
      ...
      dlBtn.addEventListener('click', function() {
        ...
        fetch(cfg.reportAction, { method: 'POST', body: fd })
          .then(function(r) {
            return r.text().then(function(text) {
              ...
              try {
                ...
                throw new Error((i18n.reportError || 'Error generating report') + ': ' + text.substring(0, 300));
              }
              if (!r.ok || !data.ok) {
                throw new Error(data.error || (i18n.reportError || 'Error generating report'));
              }
              return data;
            });
          })
          .then(function(data) {
            if (!data.data) throw new Error(i18n.reportError || 'Error generating report');
    ...
    function processNext(offset) {
      ...
      fd.append('html',             _queuePayload.html || '');
      fd.append('plain',            _queuePayload.plain || '');
      fd.append('attachments_b64',  JSON.stringify(_queuePayload.attachments_b64 || []));
      fd.append('inline_images_b64', JSON.stringify(_queuePayload.inline_images_b64 || []));
    ...
    function startSend() {
      ...
      _sendId  = '';
      _queuePayload = { html: '', plain: '', attachments_b64: [], inline_images_b64: [] };
      _startTime = Date.now();
        ...
        doFetch(fd,
          function(data) {
            ...
            _queuePayload = {
              html: data.html || '',
              plain: data.plain || '',
              attachments_b64: Array.isArray(data.attachments_b64) ? data.attachments_b64 : [],
              inline_images_b64: Array.isArray(data.inline_images_b64) ? data.inline_images_b64 : []
            };
